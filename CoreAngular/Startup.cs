...
namespace CoreAngular
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            services.AddSwaggerGen(c =>
            {
                c.SwaggerDoc("v1", new Info
                {
                    Title = "My API",
                    Version = "v1",
                    Description = "An ASP.NET Core Web API for angular",
                    Contact = new Contact { Name = "Chester Yang", Email = "", Url = "https://github.com/chesteryang" }
                });
                // Set the comments path for the Swagger JSON and UI.
                var basePath = AppContext.BaseDirectory;
                var xmlPath = Path.Combine(basePath, "CoreAngular.xml");
        ...
        public void Configure(IApplicationBuilder app, IHostingEnvironment env)
        {
            ...
            app.UseSwaggerUI(c =>
            {
                c.SwaggerEndpoint("/swagger/v1/swagger.json", "My API for angular V1");
            });
            app.UseMvc(routes =>
            {
                routes.MapRoute(
